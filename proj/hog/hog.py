...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    tmp = 0
    if num_rolls > 0:
        tmp = roll_dice(num_rolls, dice)
    else:
        tmp = oink_points(player_score, opponent_score)
    return tmp
    # END PROBLEM 3
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        leader = 0
        if last_leader != 0:
            message = "Player 0 takes the lead by %d" % (score0-score1)
    elif score0 < score1:
        leader = 1
        if last_leader != 1:
            message = "Player 1 takes the lead by %d" % (score1-score0)
    else:
        leader = None
        message = None
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if pigs_on_prime(score+oink_points(score,opponent_score), opponent_score) > 0 or oink_points(score, opponent_score) >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
